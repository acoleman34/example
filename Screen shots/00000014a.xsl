...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:seagull="http://www.seagullsw.com/legasuite/1.0">
    ...
    <xsl:element name="result">
        <xsl:choose>
            <xsl:when test="normalize-space(translate(*/seagull:field[@name='FLD12002']/@value,$lcletters,$ucletters)) = '4'">
            ...
            <xsl:when test="seagull:action/@key = 'f3'">
             ...
             <xsl:when test="seagull:action/@key = 'f4'">
                <xsl:choose>
                   <xsl:when test="seagull:cursor/seagull:position/@x='13'">
                      <xsl:attribute name="screen">00000018</xsl:attribute>
                   </xsl:when>
                   <xsl:when test="seagull:cursor/seagull:position/@x='44'">
                      <xsl:attribute name="screen">000000234</xsl:attribute>
                   </xsl:when>
                   <xsl:when test="seagull:cursor/seagull:position/@x='6'">
                      <xsl:attribute name="screen">00000015</xsl:attribute>
                   </xsl:when>
                   <xsl:otherwise>00000014a</xsl:otherwise>
                   </xsl:choose>
                   </xsl:when>
            <xsl:otherwise>
