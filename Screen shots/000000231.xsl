...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:seagull="http://www.seagullsw.com/legasuite/1.0">
    ...
    <xsl:element name="result">
        <xsl:choose>
            ...
            <xsl:when test="normalize-space(translate(*/seagull:field[@name='FLD13003']/@value,$lcletters,$ucletters)) = '1'">
                      <xsl:attribute name="screen">00000016a</xsl:attribute>
                   </xsl:when>
            <xsl:when test="normalize-space(translate(*/seagull:field[@name='FLD14003']/@value,$lcletters,$ucletters)) = '1'">
                      <xsl:attribute name="screen">00000016</xsl:attribute>
                   </xsl:when>
