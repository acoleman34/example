    ...
    <xsl:element name="result">
        <xsl:choose>
            <xsl:when test="seagull:action/@key = 'enter'">
                <xsl:choose>
                   <xsl:when test="normalize-space(translate(*/seagull:field[@name='FLD20007']/@value,$lcletters,$ucletters)) = 'OEMENU'">
                      <xsl:attribute name="screen">00000001</xsl:attribute>
                   </xsl:when>
                   <xsl:when test="normalize-space(translate(*/seagull:field[@name='FLD20007']/@value,$lcletters,$ucletters)) = '90'">
